...
async def get_file_info(
    ...
    output = await check_output(
        *prepare_mkvtoolnix_path(mkvmerge_path), '-J', file_path,
    )
    return json.loads(output)

verified_mkvmerge: set = set()

async def verify_mkvmerge(mkvmerge_path: str | list | os.PathLike | tuple[str, ...] | None = "mkvmerge") -> bool:
    """
    Parameters
    ----------
    ...
    """
    if mkvmerge_path in verified_mkvmerge:
        return True
    try:
        mkvmerge_command = [*prepare_mkvtoolnix_path(mkvmerge_path), "-V"]
        output = await check_output(*mkvmerge_command)
        output = output.decode()  # noqa: S603
    except (sp.CalledProcessError, FileNotFoundError):
        return False
    ret = bool(match("mkvmerge.*", output))
    if ret:
        verified_mkvmerge.add(mkvmerge_path)
    return ret


async def verify_matroska(file_path: str | os.PathLike, mkvmerge_path: str | list | os.PathLike | None = "mkvmerge") -> bool:
    """
    Parameters
    ----------
    ...
    """
    if not await verify_mkvmerge(mkvmerge_path=mkvmerge_path):
    ...
    try:
        ...
        info_json: dict = await get_file_info(file_path, mkvmerge_path)
...
async def verify_file_path_and_mkvmerge(
    file_path: str,
...
async def verify_recognized(file_path: str, mkvmerge_path: str | None = "mkvmerge"):  # noqa: ANN201
    """
    Parameters
    ----------
    file_path : str
    ...
    """
    file_path = await verify_file_path_and_mkvmerge(file_path, mkvmerge_path)
    try:
        if isinstance(mkvmerge_path, list):
            mkvmerge_path = tuple(mkvmerge_path)
        info_json = await get_file_info(file_path, mkvmerge_path, check_path=False)
    except sp.CalledProcessError as e:
        msg = f'"{file_path}" could not be opened'
        raise ValueError(msg) from e
...
async def verify_supported(  # noqa: ANN201
    file_path: str,
    mkvmerge_path: str | list | tuple[str, ...] | os.PathLike | None = "mkvmerge",
):
    """
