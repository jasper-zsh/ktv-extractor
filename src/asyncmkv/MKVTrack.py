...
from asyncmkv.utils import ensure_info, prepare_mkvtoolnix_path
from asyncmkv.Verifications import checking_file_path, get_file_info, verify_supported
...
class MKVTrack:
    ...
    async def new(  # noqa: PLR0913
        ...
        await self.set_file_path(file_path)
        self._track_id = None
    ...
    async def set_file_path(self, file_path: str) -> None:
        """
    ...
    async def extract(self, output_path: str | os.PathLike | None = None, silent: bool | None = False) -> str:
        """
        ...
        if silent:
            await subprocess.run(command, stdout=open(devnull, 'wb'), check=True)  # noqa: S603, PTH123, SIM115
        else:
            await subprocess.run(command, check=True, capture_output=True)  # noqa: S603
        return output_path
