...
from asyncmkv.MKVAttachment import MKVAttachment
from asyncmkv.MKVTrack import MKVTrack
from asyncmkv.Timestamp import Timestamp
from asyncmkv.utils import prepare_mkvtoolnix_path
from asyncmkv.Verifications import checking_file_path, get_file_info, verify_mkvmerge, verify_supported
...
class MKVFile:
    ...
    """
    @classmethod
    async def new(  # noqa: C901, PLR0912
        cls,
        ...
        if file_path is not None:
            ...
            for track in info_json["tracks"]:
                ...
                await self.add_track(new_track, new_file=False)
    ...
    async def mux(self, output_path: str | os.PathLike, silent: bool = False) -> int:
        """
        ...
        proc = await asyncio.create_subprocess_exec(*args, stdout=stdout, stderr=stderr)  # noqa: S603
    ...
    async def add_file(self, file: MKVFile | str | os.PathLike) -> None:
        """
        ...
        if isinstance(file, (str, os.PathLike)):
            self._number_file += 1
            new_tracks = await MKVFile(file).tracks
    ...
    async def add_track(self, track: str | MKVTrack, new_file: bool = True) -> None:
        """
        ...
        if isinstance(track, str):
            new_track = await MKVTrack.new(track, mkvmerge_path=self.mkvmerge_path, existing_info=self._info_json)
